...
int main(int argc, char **argv){
    ...
    CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&numBlocksPerSm, parallel_bubble_sort, threadPerBlock, 0));
    ...
    //we can modulate the number of columns, by taking the smallest
    ...
    while(num_blocchi>maxBlocksPerKernel){
        colonne *= 2; //doule the number of columns and try again
        righe = (dim_array+colonne-1)/colonne;
        num_blocchi = (righe + block.x*block.y - 1) / (block.x*block.y);
    }

    printf("Num righe %d\n Num Colonne %d\n\n", righe, colonne);
    int gridDimX = ceil(sqrt((double)num_blocchi)); // if it is not a perfect square approximate by excess
    ...
    //per test: array to sort on GPU Pinned vs Pageable
    int *h_matrix;
    h_matrix = (int *)malloc(dim_array* sizeof(int));
    //CHECK(cudaMallocHost((float **)&h_matrix, dim_array* sizeof(int)));
    ...
    int smemSize = block.x * block.y * colonne * sizeof(int);
    cudaFuncSetAttribute(parallel_bubble_sort, cudaFuncAttributeMaxDynamicSharedMemorySize, smemSize);

    //inaccurate calculation of execution time on device, just for test the speedup
    clock_t gpu_start = clock();
    CHECK(cudaLaunchCooperativeKernel(
        (void*)parallel_bubble_sort,   // puntatore al kernel
        grid,                 // dim3 grid
        block,                // dim3 block
    ...
    //in the place of
    //parallel_bubble_sort <<<grid, block>>>(d_matrix, STANDARD_COLUMN, righe, dim_array, num_blocchi);
